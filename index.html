...
<body>
    ...
    <div class="header">
        <h1><i class="fas fa-plug"></i> CC with Date</h1>
    ...
    <script>
        ...
        // Extract date from timestamp (handles various formats with extra spaces)
        function extractDateFromTimestamp(timestamp) {
            if (!timestamp) return 'N/A';
            
            // Clean up the timestamp by removing extra spaces
            timestamp = timestamp.trim().replace(/\s+/g, ' ');
            
            // Try to match date pattern (YYYY-MM-DD)
            const dateMatch = timestamp.match(/(\d{4}-\d{2}-\d{2})/);
            if (dateMatch) {
                return dateMatch[0]; // Return just the date part
            }
        ...
        function addScrapData() {
            ...
            scrapData = scrapRows.map(row => {
                ...
                return {
                    ...
                    contactDate: 'N/A' // Initialize contact date as N/A
                };
            });
        ...
        function processCCData() {
            ...
            // Parse CC data to extract phone numbers, staff IDs, and contact dates
            ...
            ccRows.forEach(row => {
                ...
                // Try to find contact start time (usually in column C or similar)
                let contactDate = 'N/A';
                for (let i = 0; i < Math.min(columns.length, 5); i++) {
                    if (columns[i] && columns[i].match(/\d{4}-\d{2}-\d{2}/)) {
                        contactDate = extractDateFromTimestamp(columns[i]);
                        break;
                    }
                }
                ...
                if (matchingItem) {
                    if (staffId !== null && obAgentIDs.has(staffId)) {
                        ...
                        matchingItem.contactDate = contactDate; // Set contact date for closed numbers
                        processedCount++;
                        ...
                        matchingItem.contactDate = 'N/A'; // Keep as N/A for open numbers
                        processedCount++;
                    }
                }
            });
        ...
        function renderTable(data) {
            ...
            data.forEach((item, index) => {
                ...
                // Add contact date cell
                const dateCell = document.createElement('td');
                dateCell.textContent = item.contactDate;
